# GitHub Copilot Instructions for T2 Repository
...
This is a large-scale Python project with multiple packages and modules. When providing suggestions, reference the following packages as examples of our preferred coding style:

- **gt:envoy** - Qt-related utilities and UI components
- **gt:globals** - Global utilities and shared functionality
- **gt:pythoncore** - Python utilities and helper functions

These packages should be used as reference for coding patterns, structure, and style, but don't need to be followed exactly.
...
For better VS Code IntelliSense rendering, use Markdown-style code fences within docstrings:
...
**Multiple Examples with Markdown Formatting:**

Use bullet points (`-`) with reStructuredText code directive (`::`) and Markdown code fences:
...
**Why this format:**
- The `::` at the end of bullet points signals a code block (reStructuredText)
- Markdown code fences (` ```python `) provide syntax highlighting in VS Code
- Bullet points allow multiple distinct examples in a clean format
- VS Code IntelliSense will render these with proper formatting and syntax highlighting
